...
use Test::Most;    # provides is, throws_ok, is_deeply — Test::Deep not needed
...
subtest 'comma normalization (internal branch)' => sub {
	is(abbreviate('Adams, John Quincy'), 'J. Q. Adams', 'last, first normalized');
	is(abbreviate('Adams, John'),        'J. Adams',    'two-part comma name');
	done_testing();
};

subtest 'leading comma edge case ($had_leading_comma)' => sub {
	is(abbreviate(', John Quincy'),                    'J. Q.', 'leading comma drops last name');
	is(abbreviate(', John', { format => 'initials' }), 'J.',    'leading comma initials only');
	done_testing();
};

subtest 'whitespace normalization (internal behavior)' => sub {
	is(abbreviate('  John   Quincy   Adams  '),     'J. Q. Adams', 'extra whitespace normalized');
	is(abbreviate("Adams,   John   Quincy  "),       'J. Q. Adams', 'whitespace with comma');
	done_testing();
};

subtest 'initial extraction edge cases' => sub {
	is(abbreviate('J Q Adams'),    'J. Q. Adams', 'single-letter names preserved');
	is(abbreviate('J. Q. Adams'),  'J. Q. Adams', 'pre-initialized input tolerated');
	done_testing();
};
...
subtest 'empty and degenerate input paths' => sub {
	...
	is(abbreviate(' , '), '', 'comma/whitespace normalizes to empty string');

	done_testing();
};

subtest 'validation errors' => sub {
	throws_ok(sub { abbreviate(undef) },                        qr/name/,   'undef name rejected');
	throws_ok(sub { abbreviate('John', { format => 'invalid' }) }, qr/format/, 'invalid format rejected');
	throws_ok(sub { abbreviate('John', { style  => 'invalid' }) }, qr/style/,  'invalid style rejected');
	done_testing();
};
